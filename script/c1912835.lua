...
function s.doubtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.FaceupFilter(s.filter),tp,LOCATION_MZONE,0,1,nil) 
		or Duel.IsExistingMatchingCard(aux.FaceupFilter(s.filter2),tp,0,LOCATION_MZONE,1,nil) end
end
function s.doubop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.IsExistingMatchingCard(aux.FaceupFilter(s.filter2),tp,0,LOCATION_MZONE,1,nil) then
		...
		if op==0 then
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATTACK)
			local tc=Duel.SelectMatchingCard(tp,aux.FaceupFilter(s.filter2),tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
			e:SetLabel(tc:GetFieldID())
			if tc then
				--double atk
				local e1=Effect.CreateEffect(c)
